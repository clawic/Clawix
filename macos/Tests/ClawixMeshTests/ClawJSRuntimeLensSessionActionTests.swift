...
final class ClawJSRuntimeLensSessionActionTests: XCTestCase {
    func testHermesRuntimePortalSessionActionsCommandsAndOverlays() async throws {
        ...
        XCTAssertEqual(actionPresentation.rows.first { $0.action == "send" }?.officialTransportSurface, "stdio_or_websocket_json_rpc")
        XCTAssertEqual(actionPresentation.rows.first { $0.action == "send" }?.productionTransportBlocker, "approval_required_for_non_loopback_endpoint_and_lifecycle_management")
        ...
        XCTAssertTrue(actionPresentation.rows.first { $0.action == "send" }?.accessibilityLabel.contains("official transport stdio_or_websocket_json_rpc") == true)
        ...
        XCTAssertTrue(actionPresentation.rows.first { $0.action == "send" }?.detailLabel?.contains("official transport stdio_or_websocket_json_rpc") == true)
        ...
        XCTAssertEqual(contractPresentation.rows.first { $0.action == "create" }?.officialTransportSurface, "stdio_or_websocket_json_rpc")
        XCTAssertEqual(contractPresentation.rows.first { $0.action == "create" }?.productionTransportBlocker, "approval_required_for_non_loopback_endpoint_and_lifecycle_management")
        ...
        XCTAssertEqual(commandPresentation.rows.first?.officialTransportSurface, "stdio_or_websocket_json_rpc")
        XCTAssertEqual(commandPresentation.rows.first?.productionTransportBlocker, "approval_required_for_non_loopback_endpoint_and_lifecycle_management")
        ...
        XCTAssertTrue(commandPresentation.rows.first?.accessibilityLabel.contains("official transport stdio_or_websocket_json_rpc") == true)
    ...
    func testRuntimeLensClientSurfacesHermesProductionEndpointPolicyBlock() async throws {
        ...
        XCTAssertTrue(details.contains {
            $0.contains("official contract required false")
                && $0.contains("official contract known true")
                && $0.contains("integration required true")
                && $0.contains("fixture required true")
        })
        XCTAssertTrue(details.contains {
            $0.contains("endpoint policy non_loopback_endpoint_rejected_until_production_transport_lifecycle_policy")
                ...
                && $0.contains("write back blocked_until_tui_gateway_wrapper_fixture")
                ...
                && $0.contains("claim effect blocks_recommended_production_native_parity")
                && $0.contains("support resolution explicitly_product_blocked_not_a_silent_gap")
                && $0.contains("evidence hermes.sessions.send.action_contract")
                && $0.contains("reentry blocked_until_tui_gateway_wrapper_fixture")
                && $0.contains("action command runtime hermes sessions send --session-key <id> --message <text> --confirm-runtime-write --json")
                && $0.contains("required evidence tui_gateway_prompt_submit_fixture, non_destructive_fixture, confirmation_or_dry_run_policy, round_trip_native_visibility")
                && $0.contains("risk controls no_silent_runtime_write, no_direct_runtime_store_mutation, local_overlay_only_until_contract_exists")
