...
final class ClawJSRuntimeLensSessionActionTests: XCTestCase {
    func testHermesRuntimePortalSessionActionsCommandsAndOverlays() async throws {
        ...
        XCTAssertEqual(actionPresentation.wouldWriteRuntimeCount, 1)
        XCTAssertEqual(actionPresentation.requiredEvidenceCount, 4)
        ...
        XCTAssertEqual(actionPresentation.rows.first { $0.action == "send" }?.writeDisposition, "no write")
        ...
        XCTAssertEqual(contractPresentation.wouldWriteRuntimeCount, 1)
        ...
        XCTAssertEqual(commandPresentation.readLocalCount, 19)
