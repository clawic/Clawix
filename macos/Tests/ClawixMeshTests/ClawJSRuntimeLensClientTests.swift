...
final class ClawJSRuntimeLensClientTests: XCTestCase {
    ...
    func testRuntimeLensDecodesHermesRuntimePortalEnvelope() async throws {
        ...
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.id, "hermes-sqlite-session")
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.nativeIdentifier?.name, "sessionId")
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.provenance?.source, "runtime-session-sqlite")
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.parentSessionId, "parent-session")
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.inputTokens, 11)
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.outputTokens, 23)
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.cacheReadTokens, 5)
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.cacheWriteTokens, 7)
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.reasoningTokens, 13)
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.billingProvider, "openai")
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.billingMode, "api_key")
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.estimatedCostUsd, 0.0123)
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.actualCostUsd, 0.0101)
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.costStatus, "estimated")
        XCTAssertEqual(snapshot.resources(for: "sessions").first?.apiCallCount, 2)
