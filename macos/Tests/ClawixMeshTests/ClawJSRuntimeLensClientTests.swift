...
final class ClawJSRuntimeLensClientTests: XCTestCase {
    ...
    func testRuntimeLensDecodesHermesRuntimePortalEnvelope() async throws {
        ...
        XCTAssertEqual(snapshot.officialSnapshot?.capturedAt, "2026-05-25")
        XCTAssertEqual(snapshot.officialSnapshot?.sourceSnapshotDate, "2026-05-25")
        ...
        XCTAssertEqual(snapshot.officialSnapshot?.driftPolicy, "hermes_remains_dev_only_until_snapshot_total_and_write_policy_are_complete")
        ...
        XCTAssertEqual(snapshot.support?.ecosystem?.summary?.contains("native write-back contracts"), true)
        XCTAssertEqual(snapshot.support?.ecosystem?.summary?.contains("approval-gate receipts"), true)
        XCTAssertEqual(snapshot.support?.ecosystem?.summary?.contains("TUI Gateway production transport policy"), true)
        XCTAssertEqual(snapshot.support?.ecosystem?.summary?.contains("channel/provider/auth/model evidence"), true)
        ...
        XCTAssertEqual(snapshot.support?.ecosystem?.officialSnapshot?.capturedAt, "2026-05-25")
        ...
        XCTAssertEqual(snapshot.supportAudit?.officialSnapshot?.capturedAt, "2026-05-25")
        XCTAssertEqual(snapshot.supportAudit?.provenance?.officialSnapshotSource, "docs/runtime-ecosystem-integration.manifest.json")
        XCTAssertEqual(snapshot.supportAudit?.provenance?.sourceSnapshotDate, "2026-05-25")
        let hermesSupportOverview = ClawJSRuntimeLensSupportOverviewPresentation.make(support: try XCTUnwrap(snapshot.support))
        XCTAssertEqual(hermesSupportOverview.blockingReasonCount, 9)
        XCTAssertEqual(hermesSupportOverview.officialSnapshotCapturedAt, "2026-05-25")
        XCTAssertEqual(hermesSupportOverview.officialSnapshotSourceSnapshotDate, "2026-05-25")
        XCTAssertEqual(hermesSupportOverview.officialSnapshotSourceCount, 8)
        XCTAssertEqual(hermesSupportOverview.officialSnapshotLabel, "captured 2026-05-25, source snapshot 2026-05-25, sources 8")
        XCTAssertEqual(hermesSupportOverview.officialSnapshotDriftPolicy, "hermes_remains_dev_only_until_snapshot_total_and_write_policy_are_complete")
        XCTAssertTrue(hermesSupportOverview.accessibilityLabel.contains("official snapshot 2026-05-25"))
        ...
        XCTAssertTrue(hermesSupportOverview.summary?.contains("native write-back contracts") == true)
        XCTAssertTrue(hermesSupportOverview.summary?.contains("approval-gate receipts") == true)
        XCTAssertTrue(hermesSupportOverview.summary?.contains("TUI Gateway production transport policy") == true)
        XCTAssertTrue(hermesSupportOverview.summary?.contains("channel/provider/auth/model evidence") == true)
