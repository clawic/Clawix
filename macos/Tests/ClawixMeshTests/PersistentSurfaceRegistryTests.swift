...
final class PersistentSurfaceRegistryTests: XCTestCase {
    func testClawixPersistentSurfaceRegistryCoversLocalDatabaseAndPrefs() throws {
        ...
        XCTAssertTrue(nodes.contains { $0.id == "clawix.prefs.legal.acceptedSafetyVersion" && $0.key == "Legal.AcceptedSafetyVersion" })
        XCTAssertTrue(nodes.contains { $0.id == "clawix.prefs.legal.acceptedRegulatedDomainsVersion" && $0.key == "Legal.AcceptedRegulatedDomainsVersion" })
        XCTAssertTrue(nodes.contains { $0.id == "clawix.prefs.legal.sensitiveExportConfirmationRequired" && $0.key == "Legal.SensitiveExportConfirmationRequired" })
        XCTAssertTrue(nodes.contains { $0.id == "clawix.prefs.legal.localAuditRetentionDays" && $0.key == "Legal.LocalAuditRetentionDays" })
        XCTAssertTrue(nodes.contains { $0.id == "clawix.prefs.apps.variantDefaults" && $0.key == "clawix.apps.variantDefaults.v1" })
        XCTAssertTrue(nodes.contains { $0.id == "clawix.prefs.systemTelemetry.menuBarWidgets" && $0.key == "SystemTelemetry.MenuBar.EnabledWidgetIDs" })
