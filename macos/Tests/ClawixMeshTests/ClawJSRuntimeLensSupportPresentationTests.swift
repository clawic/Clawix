...
final class ClawJSRuntimeLensSupportPresentationTests: XCTestCase {
    ...
    func testRuntimeLensSupportAuditEvidenceAndPromotionPresentations() async throws {
        ...
        XCTAssertEqual(auditEvidencePresentation.rows.first?.resolutionLabel, "explicitly_product_blocked_not_a_silent_gap, native_session_action_unsupported_until_official_runtime_contract, blocks_recommended_production_native_parity")
        XCTAssertEqual(auditEvidencePresentation.rows.first?.expectedEvidenceLabel, "official_create_command_or_api, non_destructive_fixture")
        XCTAssertEqual(auditEvidencePresentation.rows.first?.riskControlsLabel, "no_silent_runtime_write")
        XCTAssertTrue(auditEvidencePresentation.rows.first?.detailLines.contains("fallback do_not_synthesize_native_runtime_action") == true)
        XCTAssertTrue(auditEvidencePresentation.rows.first?.detailLines.contains("expected evidence official_create_command_or_api, non_destructive_fixture") == true)
        XCTAssertTrue(auditEvidencePresentation.rows.first?.detailLines.contains("risk controls no_silent_runtime_write") == true)
        ...
        XCTAssertTrue(reentryPresentation.rows.last?.accessibilityLabel.contains("official transport stdio_or_websocket_json_rpc") == true)
        XCTAssertTrue(reentryPresentation.rows.last?.accessibilityLabel.contains("production blocker approval_required_for_non_loopback_endpoint_and_lifecycle_management") == true)
