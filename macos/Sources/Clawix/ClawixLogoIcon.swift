...
/// Clawix brand logo / Agents glyph: outer iOS-style continuous-corner
/// squircle with the visor cut out and two squircle eyes filled back in.
///
/// Two appearance variants:
/// - Dark mode: the solid filled mask (single evenodd path), unchanged.
/// - Light mode: the inverse the user asked for — the outer squircle drawn
///   as a *border*, the visor filled black as the mask, and the eyes left
///   transparent (cut out of the visor).
enum ClawixLogoLightStyle {
    /// Solid filled mask (the brand mark, just inked dark on light).
    case fill
    /// A much thinner outer border + the visor mask with transparent eyes.
    case thinBorder
}

struct ClawixLogoIcon: View {
    var size: CGFloat = 18
    /// Only consulted in light mode; dark mode is always the solid mask.
    var lightStyle: ClawixLogoLightStyle = .fill
    ...
    var body: some View {
        Group {
            if scheme == .light {
                ...
                case .thinBorder:
                    ZStack {
                        ClawixLogoOuterShape()
                            .stroke(.primary, style: StrokeStyle(
                                lineWidth: 4.0 * (size / 100),
                                lineJoin: .round
                            ))
                        ClawixLogoVisorEyesShape()
                            .fill(.primary, style: FillStyle(eoFill: true))
                    }
                    .padding(size * 0.03)
                }
...
/// Template `NSImage` for AppKit tinting (menu bar, dock badges). Always the
/// solid mask — the menu bar applies its own foreground colour.
...
// MARK: - Geometry pieces (100x100 design space)

private func clawOuter(into path: inout Path, _ p: (CGFloat, CGFloat) -> CGPoint) {
    path.move(to: p(38, 0))
...
private func clawVisor(into path: inout Path, _ p: (CGFloat, CGFloat) -> CGPoint) {
...
private func clawEyes(into path: inout Path, _ p: (CGFloat, CGFloat) -> CGPoint) {
    // Left eye.
...
private func clawMapper(_ rect: CGRect) -> (CGFloat, CGFloat) -> CGPoint {
...
struct ClawixLogoShape: Shape {
    func path(in rect: CGRect) -> Path {
        let p = clawMapper(rect)
        var path = Path()
        clawOuter(into: &path, p)
        clawVisor(into: &path, p)
        clawEyes(into: &path, p)
        return path
    }
}

/// Just the outer squircle — stroked as the border in the light variant.
struct ClawixLogoOuterShape: Shape {
    func path(in rect: CGRect) -> Path {
        let p = clawMapper(rect)
        var path = Path()
        clawOuter(into: &path, p)
...
/// Visor band with the eyes cut out (even-odd) — the black mask of the
/// light variant, eyes transparent.
struct ClawixLogoVisorEyesShape: Shape {
    func path(in rect: CGRect) -> Path {
        let p = clawMapper(rect)
        var path = Path()
        clawVisor(into: &path, p)
