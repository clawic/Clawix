...
enum ToolTimelinePresentation {
    ...
    fileprivate static func buildSnapshot(
        ...
        // Single chokepoint for every presentation-snapshot build. Counted so a
        // deterministic test can assert that opening a large session builds ZERO
        // snapshots before any tool group is expanded (law #5: cheap first
        // frame; heavy detail materializes lazily on render/expand).
        RenderProbe.count("tool.snapshot.build", by: 1, recordsActivity: false)
