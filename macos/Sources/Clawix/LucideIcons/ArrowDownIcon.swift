...
/// Lucide `arrow-down` glyph, ported 1:1 from the lucide-icons/lucide
...
struct ArrowDownIconShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        var path = Path()
        path.addPath(SVGPathBuilder.build("M12 5v14"), transform: xform)
        path.addPath(SVGPathBuilder.build("m19 12-7 7-7-7"), transform: xform)
        return path
    }
}
