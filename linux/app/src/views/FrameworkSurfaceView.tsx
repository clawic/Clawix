...
import { frameworkSurfaceById, frameworkSurfaces, type FrameworkSurfaceId } from "../lib/framework_surface_model";

export default function FrameworkSurfaceView() {
  const [selected, setSelected] = createSignal<FrameworkSurfaceId>("claw");
  const surface = () => frameworkSurfaceById(selected());

  return (
    <section class="h-full overflow-auto">
      <div class="mx-auto max-w-[720px] px-8 py-10">
        <header class="mb-7">
          <h1 class="text-[22px] font-semibold tracking-tightish">Claw</h1>
          <p class="mt-1 text-sm text-zinc-500">
            Framework status, mesh targets, and git affordances stay explicit about owner, policy, and validation.
          </p>
        </header>

        <section class="mb-7">
          <h2 class="mb-2 text-[11px] font-semibold uppercase text-zinc-500">Surfaces</h2>
          <div class="grid gap-2 md:grid-cols-3">
            <For each={frameworkSurfaces}>
              {(item) => (
                <button
                  type="button"
            ...
            <For each={surface().commands}>
              {(command, index) => (
                <>
                  ...
                  <Row label={command}>
                    <code class="max-w-[360px] break-all text-right text-xs text-zinc-500">{command}</code>
                  </Row>
                </>
              )}
            </For>
