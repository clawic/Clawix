...
(function () {
  ...
  /* base prefix: "" at the style/ root, "../" inside a subfolder (lab/).
     Every generated href and icon-sprite ref is prefixed with it so the
     same app.js drives pages at any depth without broken links. */
  var base = body.getAttribute("data-base") || "";
  var ic = function (n, cls) { return '<svg class="ic' + (cls ? ' ' + cls : '') + '"><use href="' + base + 'icons.svg#i-' + n + '"/></svg>'; };
  ...
  var REF = [
    ...
    { id: "icons", label: "Iconography", href: "icons.html" },
    { id: "lab", label: "Lab", href: "lab/index.html" }
  ];
  var SCREENS = [
  ...
  SCREENS.forEach(function (s) {
    ...
    screensMenu += '<a class="it" href="' + base + s.href + '" style="text-decoration:none' + (sel ? ";background:var(--sel)" : "") + '">' + s.label +
      (sel ? '<span class="ck">' + ic("check", "xs") + "</span>" : "") + "</a>";
  });
  var nav = document.createElement("div");
  nav.className = "topbar";
  nav.innerHTML =
    '<a class="brand" href="' + base + 'index.html" style="text-decoration:none">Clawix <span>· Style System</span></a>' +
    '<div class="nav-links">' +
      REF.map(function (p) {
        return '<a class="nav-link' + (p.id === current ? " on" : "") + '" href="' + base + p.href + '">' + p.label + "</a>";
      }).join("") +
