...
function assertCompletionAudit() {
  ...
  for (const snippet of [
    ...
    "`jobs.stream`, `jobs.start`, and `jobs.cancel` are visible only as blocked explicit gaps",
    "Sibling ClawJS runtime now has local authenticated backend routes for `runtime/jobs/start`",
    "Clawix still blocks `window.clawix.jobs.stream/start/cancel`",
    ...
    "Signed-app UI/Instruments performance evidence still needs an approved baseline and reviewable closure summary",
...
function assertPublicRouting() {
  for (const [relativePath, snippets] of Object.entries({
    ...
    "docs/governance/sdk-first-custom-surfaces/plan.md": [
      ...
      "`jobs.stream` is present only as a blocked explicit gap",
