...
const isSelfTest = process.env.CLAWIX_UI_DRIFT_VERIFY_SELF_TEST === "1";
const errors = [];
...
function runFailureSelfTests() {
  const selfTestEnv = {
    ...process.env,
    CLAWIX_UI_DRIFT_VERIFY_SELF_TEST: "1",
    CLAWIX_UI_ALLOW_PENDING_PRIVATE_EVIDENCE: "",
  };
  const missingRoot = path.join(os.tmpdir(), `clawix-ui-private-drift-missing-${process.pid}`);
  const tests = [
    ...
    [["--require-approved", "--root", rootDir], "private drift root must be outside the public repository"],
    [["--require-approved", "--root", missingRoot], `private drift root does not exist: ${path.resolve(missingRoot)}`],
  ];
  ...
  for (const [testArgs, expectedOutput] of tests) {
    ...
    if (result.status === 0) {
      fail(`self-test ${testArgs.join(" ") || "<no args>"} must fail for private drift verification`);
      continue;
    }
