...
function assertExternalValidationManifest() {
  ...
  assert(manifest.externalClosureFixtures?.invalidMutationCount === 8, "external validation manifest: wrong external closure invalid mutation count");
  assert(manifest.externalClosureFixtures?.closureRole?.includes("approval id, approval window, and exact closure rows"), "external validation manifest: external closure fixtures closure role must be explicit");
...
function assertExternalClosureFixtures() {
  ...
  assert(Array.isArray(fixtures.invalidSyntheticMutations) && fixtures.invalidSyntheticMutations.length === 8, "external closure fixtures: must contain 8 invalid mutations");
  for (const rowId of ["CLX-SYS-TEL-EXT-003", "CLX-SYS-TEL-EXT-004", "CLX-SYS-TEL-EXT-005"]) {
...
function assertExternalClosureValidator() {
  ...
  assert(result.invalidSyntheticMutations === 8, "external closure validator: must reject 8 invalid synthetic mutations");
  for (const rowId of ["CLX-SYS-TEL-EXT-003", "CLX-SYS-TEL-EXT-004", "CLX-SYS-TEL-EXT-005"]) {
