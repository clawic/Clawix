...
function assertExternalValidationManifest() {
  ...
  assert(manifest.externalClosureFixtures?.invalidMutationCount === 11, "external validation manifest: wrong external closure invalid mutation count");
  assert(manifest.externalClosureFixtures?.closureRole?.includes("approval id, approving actor, credential/native grants"), "external validation manifest: external closure fixtures closure role must be explicit");
...
function assertExternalClosureFixtures() {
  ...
  assert(Array.isArray(fixtures.invalidSyntheticMutations) && fixtures.invalidSyntheticMutations.length === 11, "external closure fixtures: must contain 11 invalid mutations");
  for (const rowId of ["CLX-SYS-TEL-EXT-003", "CLX-SYS-TEL-EXT-004", "CLX-SYS-TEL-EXT-005"]) {
...
function assertExternalClosureValidator() {
  ...
  assert(result.invalidSyntheticMutations === 11, "external closure validator: must reject 11 invalid synthetic mutations");
  for (const rowId of ["CLX-SYS-TEL-EXT-003", "CLX-SYS-TEL-EXT-004", "CLX-SYS-TEL-EXT-005"]) {
