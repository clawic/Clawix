...
if (!exists(hermesFixturePath)) {
  ...
  requireEqual(supportAudit?.projectionSummary?.byReadProjectionStatus?.projected, 2, "Hermes fixture projected read-projection count");
  requireEqual(supportAudit?.projectionSummary?.byReadProjectionStatus?.degraded_projection, 11, "Hermes fixture degraded read-projection count");
  requireEqual(supportAudit?.projectionSummary?.implementedFacetCounts?.ready_runtime_projection, 1, "Hermes fixture ready runtime projection count");
  requireEqual(supportAudit?.projectionSummary?.implementedFacetCounts?.degraded_runtime_projection, 11, "Hermes fixture degraded runtime projection count");
  for (const reason of [
...
for (const file of [
  ...
  "macos/Sources/Clawix/ClawJS/ClawJSRuntimeLensSupportAuditDomainPresentation.swift",
...
for (const snippet of [
  ...
  "supportAuditDomainsSection(ClawJSRuntimeLensSupportAuditDomainPresentation.make(domains: domains))",
...
for (const snippet of [
  ...
  "ClawJSRuntimeLensSupportAuditDomainPresentation.make",
  "auditDomainPresentation.accessibilityLabel",
  ...
  "tui_gateway_wrapper_fixture_production_transport_lifecycle_policy_and_native_round_trip_evidence_attached",
  "snapshot.status.capabilityMap?[\"memory\"]?.status, \"degraded\"",
  "snapshot.domains.first { $0.domain == \"memory\" }?.status, \"degraded\"",
  ...
  "native write-back contract status blocked, writes runtime false, would write runtime false, official contract required true, official contract known false",
  "overlay receipt host runtime-portal, request appstate-test, status applied",
  ...
  "degraded reason runtime_cli_unavailable_or_session_store_missing",
