...
if (!isSelfTest && rawArgs.length === 0) {
  const selfTests = [
    ...
    ["--simulate-extra-required-flow", "requiredFlows must not include search-open"],
    ["--simulate-duplicate-required-flow", "requiredFlows duplicates sidebar-hover-click-expand"],
    ["--simulate-extra-required-metric", "requiredMetrics must not include cpuUsagePercent"],
    ["--simulate-duplicate-required-metric", "requiredMetrics duplicates interactionLatencyMs"],
    ["--simulate-extra-required-evidence-field", "requiredEvidenceFields must not include localTracePath"],
    ["--simulate-duplicate-required-evidence-field", "requiredEvidenceFields duplicates flowId"],
    ...
    ["--simulate-flow-extra-required-metric", "flows[0].requiredMetrics must not include cpuUsagePercent"],
    ["--simulate-flow-duplicate-required-metric", "flows[0].requiredMetrics duplicates interactionLatencyMs"],
    ...
    ["--simulate-approved-registry-with-pending-flow", "status cannot be approved-baseline-enforced while 24 flow budgets are pending"],
    ["--simulate-perf-decision-missing-budget-registry", "publicEvidence must include docs/ui/performance-budgets.registry.json"],
    ["--simulate-perf-decision-missing-private-baselines", "publicEvidence must include docs/ui/private-baselines.manifest.json"],
    ["--simulate-perf-decision-missing-visual-validation", "publicEvidence must include docs/ui/private-visual-validation.manifest.json"],
    ["--simulate-perf-decision-missing-evidence-plan", "publicEvidence must include scripts/ui_private_evidence_plan_check.mjs"],
