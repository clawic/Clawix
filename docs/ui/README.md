...
- Public-safe synthetic real-equivalent stress:
  ...
  equivalence claim.
- Fixture scale dimensions include reasoning/action/answer stream splits and
  multi-window or agent-instance pressure. Generated packs write
  `stream-plan.json` with explicit channels and `window-instance-plan.json`
  alongside the thread, pin, metadata churn, and terminal support artifacts.
- P1/P2 KPI coverage:
...
produced it. Each `evidenceSources` key also has a fixed expected source id and
repo-relative path; approved source ids cannot be swapped across keys.
Suite-level metric and failure artifacts must exactly match the
