...
| --- | --- | --- | --- | --- | --- | --- |
| Canon, governance, or ADR work | `docs/decision-map.md` and `docs/governance/decision-tension-rubric.md` | `CONSTITUTION.md`, `docs/constitution-map.md`, `docs/adr/TEMPLATE.md`, `docs/governance/source-decision-audits.md`, `docs/governance/source-decision-audits.registry.json`, sibling ClawJS decision map | `node scripts/constitution-sync-check.mjs`, `node scripts/source_decision_audit_check.mjs`, `node scripts/adr-operational-coverage-check.mjs`, `node scripts/discoverability-check.mjs generate --check`, focused guard named by the decision map | Do not accept a durable decision with prose only when it needs tests, guardrails, registry, source-audit closure, or CLI discovery | Decision-map row, discoverability registry, source-audit row when conversation decisions are involved, guard/test or explicit expiring debt, and source Q/A decisions implemented or blocked | `claw search query "decision tension rubric" --json`; `claw inspect why adr:decision-tension-rubric --json` |
...
| Surface, route, bridge, CLI, or protocol work | `docs/adr/0011-surface-route-graph.md` | graph source `docs/persistent-surface-clawix.manifest.json`, SidebarRoute metadata `docs/surface-route-registry.manifest.json`, sibling ClawJS ADR 0049 | `claw inspect routes --json`, `node scripts/surface-evidence-projection-check.mjs`, `node scripts/generate-surface-route-registry.mjs --check` when sidebar route metadata changes | Do not infer routes from memory; use registered `consumes`, `owns`, `exposes`, and `brokers` edges | Route, node, `contractId`, `surfaceSteward`/`steward`, tests, and gaps are registered or explicitly blocked | `claw inspect route chat.localDesktop --json`; `claw search query "surface route graph" --json` |
