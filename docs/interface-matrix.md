...
| Remote Mesh | Framework + Host | Mesh targets, pairing, trust, sharing, sync status, external-pending gaps, and route contracts | Framework Coordinator/Gateway/Connector/Sync contracts from sibling ClawJS ADR 0022, `claw inspect remote`, `RemoteExternalPendingRegister`, `RemoteRouteContractCatalog`, `SyncAuthorityHandoffReceipt`, and bridge fixtures | Framework runtime plus host state | Mesh API, remote conformance, sync manifest, authority handoff, route contract, and bridge parity tests |
...
| Composer Experiments | Host | Composer experimental affordances gated by feature visibility | Host composer feature gate and attachment/message draft APIs | Host UI state plus framework message contract | Composer visibility tests and bridge message fixture parity |
| Tools | Framework + Host | Tool timeline groups, command palette entries and tool-gated menus | Bridge V1 timeline tool entries plus host command/action policy | Framework message contract plus host policy | Bridge V1 tool timeline fixtures and feature visibility checks |
| Network Control | Framework + Host | Network control route, sidebar entry and MCP/network commands | Framework network/connector routes plus host visibility gate | Framework connector state plus host policy | Surface route registry checks and feature-gated sidebar/command tests |
