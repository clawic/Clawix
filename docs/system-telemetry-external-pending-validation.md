...
| CLX-SYS-TEL-EXT-006 | Native rendered graph view over retained telemetry | The framework Monitor store retains samples, rollups, incidents, chart-ready points, and a portable ASCII render; Clawix has a recorder path into that store and menu-bar sparklines can consume retained history. | Current Clawix native graph surface wired to Monitor history and inspected with representative retained telemetry. | EXTERNAL PENDING |
