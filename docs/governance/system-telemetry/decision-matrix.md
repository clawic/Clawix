...
| D05 | Support a combined menu-bar widget/panel in addition to independent indicators. | Validated locally. | Combined `NSStatusItem`, dropdown, provider rows, widget toggles, forced refresh action, and seeded history graph menu content. | No local blocker; verifier must continue to check the combined item. |
...
| D07 | Prepare host and app surfaces for real-time display of important information. | Validated locally. | Menu refresh path, forced `SystemTelemetryMonitorRecorder` writes for explicit refresh, bundled signed host, and live recorder smoke in `CLX-SYS-TEL-EXT-002`. | Physical/live-provider samples can only replace fixture or unavailable states through external lanes. |
| D08 | Reuse and centralize retention, charts, rules, and events in Monitor rather than creating a parallel time-series store. | Implemented and consumed by Clawix. | Recorder writes through framework Monitor; history reader consumes retained payloads, including metric purge fallback to rollups; ordered widget-metric history loading feeds menu sparklines and the AppKit graph view while operational `health_check` events stay in Monitor. | Ongoing guard: Clawix must not create a parallel system telemetry history store. |
...
| D10 | Pin the goal to the conversation id, plan id, source review, and one-by-one decision audit. | Implemented as a closure gate. | This matrix, `docs/governance/system-telemetry/completion.md`, `docs/governance/system-telemetry/source-review.json`, `docs/governance/system-telemetry/external-pending.md`, `docs/governance/system-telemetry/external-validation-runbook.md`, `docs/governance/system-telemetry/external-approval.schema.json`, synthetic approval templates in `docs/governance/system-telemetry/external-approval.fixtures.json`, approval validator `scripts/validate-system-telemetry-external-approval.mjs`, `docs/governance/system-telemetry/external-evidence.schema.json`, synthetic fixture templates in `docs/governance/system-telemetry/external-evidence.fixtures.json`, evidence validator `scripts/validate-system-telemetry-external-evidence.mjs`, same-lane closure fixtures in `docs/governance/system-telemetry/external-closure.fixtures.json`, closure validator `scripts/validate-system-telemetry-external-closure.mjs`, `docs/governance/system-telemetry/external-validation.manifest.json`, manifest schema `docs/governance/system-telemetry/external-validation.manifest.schema.json`, manifest fixtures `docs/governance/system-telemetry/external-validation.manifest.fixtures.json`, and `scripts/verify-system-telemetry-goal.mjs`. | Re-read the private source session before completion, refresh the source Q/A review, completion audit, external validation runbook, approval schema, approval fixture templates, approval validator, evidence schema, fixture templates, evidence validator, closure fixtures, closure validator, manifest schema, manifest fixtures, and this matrix if any decision changed. |
| D11 | Do not close the goal until everything is implemented, validated, documented, or explicitly re-scoped by a later user decision. | Active. | `active_goal_not_complete`, completion audit, source Q/A review, external-pending ledger, external validation manifest, external validation runbook, external evidence schema, decision matrix, and verifier. | Do not call completion while `CLX-SYS-TEL-EXT-003`, `CLX-SYS-TEL-EXT-004`, or `CLX-SYS-TEL-EXT-005` remain without approved evidence or a later explicit `scope_revision` decision. |
...
- `CLX-SYS-TEL-EXT-003`, `CLX-SYS-TEL-EXT-004`, or
  ...
  external-validation manifest as `central_promise_blocker`.
- The private source session has not been re-read for the final completion
  audit and reflected in `docs/governance/system-telemetry/source-review.json`.
