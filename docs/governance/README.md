# Goal Governance Artifacts

This directory groups public-safe goal closure artifacts by program. Keep
`docs/` for human routers, durable canon, and broad reference documents; keep
decision matrices, source audits, completion ledgers, external-pending ledgers,
plans, and companion validation manifests here.

## Programs

- [Code Hygiene](code-hygiene/completion.md)
- [Decision Tension Rubric](decision-tension-rubric.md)
- [Legal](legal/source-audit.md)
- [No Irreversible Data Loss](no-irreversible-data-loss/README.md)
- [Performance Governance](performance-governance.md)
- [Release Readiness](release-readiness.md)
- [RFC Process Mirror](../adr/0025-rfc-process-mirror.md)
- [SDK-First Custom Surfaces](sdk-first-custom-surfaces/plan.md)
- [Source Decision Audits](source-decision-audits.md)
- [System Telemetry](system-telemetry/index.md)
- [UI Governance](ui/completion.md)
- [V1 Surface Closure](v1-surface-closure/index.md)
