...
| LCA-026 | Audit retention | Local configurable | Persistent surfaces include local legal audit retention days; `LegalSafetyTests` now prove the retention value is configurable and survives reload through the persisted defaults key. | Final settings UI smoke must confirm the signed app renders the retention control. |
| LCA-027 | Provider terms | User chooses/assumes | Terms/Privacy/EULA place chosen third-party providers under their own terms; provider-backed dictation enhancement and cloud STT skip external provider clients unless the local provider disclosure opt-in is enabled; settings-source tests and the legal gate require the Provider disclosure opt-in copy and user-responsibility language. | Final signed settings UI smoke must confirm the rendered provider opt-in control. |
...
| LCA-031 | Release channels | GitHub+npm+apps+web | Clawix docs cover app/binary/web surfaces, app/Linux/Windows public copy is part of the legal claim scan, and sibling ClawJS covers npm/CLI. | Each exact channel needs separate approval. |
