...
| CLX-SDK-004 | High-risk actions interrupt only for secrets, credentials, cost, external, destructive, native-sensitive, physical/IoT, regulated, or comparable risk. | App capability risk tiers, approval-required dispatch modes, high-risk action prompt/audit receipts, `window.clawix.mac.planAction()` Mac plan-only dispatch, IoT dispatch boundary, and no plaintext secret broker runner. | Signed-host native execution and live IoT/provider actions remain externally pending until approved receipts and same-machine evidence exist. | EXTERNAL PENDING |
